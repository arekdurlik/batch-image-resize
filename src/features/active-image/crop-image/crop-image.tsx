...
export function CropImage({ thumbnailSrc, outputImageData: image, onClose }: Props) {
    ...
    useEffect(() => {
        const inputAspectRatio =
            image.inputImage.dimensions.width / image.inputImage.dimensions.height;
    ...
    function handleConfirm() {
        ...
        outputImagesApi.setCropData(image.id, {
            x: state.x,
            y: state.y,
            zoom: state.zoom,
    ...
    return (
        <>
            ...
            />
            <CropSettings onConfirm={handleConfirm} onCancel={onClose} />
        </>
    );
}
