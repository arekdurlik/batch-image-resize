...
export function AspectRatios({ variant }: { variant: Variant }) {
  ...
  const [horizontal, setHorizontal] = useState(aspectRatioIsHorizontal(variant.aspectRatio.value));
  ...
  function handleLeft() {
    ...
    api.setAspectRatioValue(variant.id, horizontal 
      ? presetAspectRatios[newIndex][0] 
      : presetAspectRatios[newIndex][1]
    );
  }
            ...
            >
              ...
              >
                <BsFillAspectRatioFill 
                  style={{ ...(!horizontal) && { transform: 'rotate(90deg) scaleX(-1)' }}}  
                  size={16}
                />
              </Button>
            </Tooltip>
