    ...
    subscribeWithSelector((set, get) => ({
        ...
        api: {
            selectItems: (items, modifiers = {}) => {
                const selectedItemsOfSameType = [...get().selectedItems].filter(
                    i => i.type === items[0]?.type
                );
            ...
            deleteSelectedItems: () => {
                try {
                    ...
                    const typeImages = type === 'input' ? inputImages : outputImages;
